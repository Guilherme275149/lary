...
const line3 = '╠══✪〘';
const line4 = '〙✪══';
const line5 = '꯴᩠ꦽꦼ𝑺͢𝒐𝒇𝒊𝒂 𝑩͢𝒐𝒕🌸';
const line6 = 'APIS DO 𝑮𝒐𝒘𝒕𝒆𝒓';
const line8 = '╠═══════════════';
const line9 = '╠══';
const line10 = 'EDITS »';
const line11 = '𝐃𝐎𝐍𝐎:';
const line12 = '*wa.me/+5511934488829*';
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo membro novo ${mdata.subject}\n\nEspero que goste do grupo leia as regras pra não ser removido❤️`
				...
				teks = `Tchau @${num.split('@')[0]} Boiola já vai tarde seu gay 😂👋`
				let buff = await getBuffer(ppimg)
		...
		if(antifake.includes(anu.jid)) {
			if (anu.action == 'add'){
				...
				if(!num.split('@')[0].startsWith(55)) {
					denz.sendMessage(mdata.id, 'Corra numero fake safado seu ban esta próximo', MessageType.text)
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD 𝑺𝒐𝒇𝒊𝒂*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Sair fora random o krlho só meu dono pode usa essa função ❌',
					ownerB: '[❗] Sair fora random o krlho só meu dono pode usa essa função  ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amiga de 𝑺𝒐𝒇𝒊𝒂. Registre-se para fazer amizade com o bot 𝑺𝒐𝒇𝒊𝒂 por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar 𝑺𝒐𝒇𝒊𝒂|17\n\n──「 𝑺𝒐𝒇𝒊𝒂 」──`,
					admin: '[❗] Koe membro comum se não tem permissão pra usar esse comando seu random😳',
					Badmin: ' [❗] Ae coloca eu como admin pow acha que sou macumbero pra vira admin do nada?😳',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5511934488829@s.whatsapp.net","12267740582@s.whatsapp.net"] // replace this with your number
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
				...
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`		
				 ...
				 data = fs.readFileSync('./dark/mortal.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
    ...
    break
		...
		case 'spambrutal':
                reply('espere um pouco ta?.....')
                                       ...
                                       if (args[0].startsWith('5521996562452')) return reply('Falha ao ligar para o número do bot')
                                       if (args[0].startsWith('5511934488829')) return reply('Falha, não é possível ligar para o número do proprietário')
...
//fotomaker
	...
	break					
                ...
                case 'gtext':
					...
					var gli = gh.split("/")[0];
					var tch = gh.split("/")[1];
					if (args.length < 1) return reply(`「❗」Exemplo : ${prefix}gtext gowter/lindo`)
					reply('pera um pokin uai')
...
//consultas			        
	                case 'dulis':
	            	if (!isPremium) return reply('Ola, esse comando e apenas para vips, para ver como se tornar vip, digite .daftarvip, 𝑮𝒐𝒘𝒕𝒆𝒓 agradece ❤️!' ,text, { quoted: mek })
					if (args.length < 1) return reply(mess.blank)
					teks = body.slice(6)
	               ...
	               case 'scor':
					...
					break	
			case 'antitravass':
			await costum(antivirtexx(), text, tescuk, `Isa bot 🌺`)
			break 
				case 'antitrava':
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO 𝑺𝒐𝒇𝒊𝒂 BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @dark domina vida
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT 𝑮𝒐𝒘𝒕𝒆𝒓 ✨* 」\n*│+ wa.me/5511934488829*╰──────*「 *𝑮𝒐𝒘𝒕𝒆𝒓* 」*────*\n\n*_SE QUER SER ADMIN DO BOT 𝑮𝒐𝒘𝒕𝒆𝒓_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
...
${line15}𝑺𝒐𝒇𝒊𝒂 𝚋𝚘𝚝 ${line4}
${line}
${line} Ola ${pushname} ☘️
${line}
${line} Bem vindo ao ${line5}
${line}
...
${line}
${line3} FOTOMAKER ${line4}
${line}
${line}277 *${prefix}gtav* (marque a foto)
${line}278 *${prefix}facebookpage* (marque a foto)
${line}279 *${prefix}costumwp* (marque a foto)
${line}280 *${prefix}pantaimalam* (marque a foto)
${line}281 *${prefix}pencil* (marque a foto)
${line}282 *${prefix}bakar* (marque a foto)
${line}283 *${prefix}crossgun* (marque a foto)
${line}
${line3} OUTROS /5 ${line4}
${line}
${line}284 *${prefix}antitravass*
${line}285 *${prefix}perfil* 
${line}286 *${prefix}detector* 
${line}
${line3} 𝗗𝗢𝗡𝗢 ${line4}
${line}
${line} *NOME: 𝑮𝒐𝒘𝒕𝒆𝒓  ☔*
${line} *INSTA: sem no momento*
${line} *WPP: wa.me/+5511934488829*
${line} *YOUTUBE: https://youtube.com/channel/UCWZKMTuAI8t4ENaCftJxqQw*
${line}
